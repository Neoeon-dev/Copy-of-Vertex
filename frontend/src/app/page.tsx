      ...
      <div>
        <Card className="overflow-hidden">
          ...
          <div className="p-5 sm:p-7">
            ...
            <motion.button type="button" onClick={() => inputRef.current?.click()} onDragOver={(e) => { e.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onDrop={(e) => { e.preventDefault(); setDragging(false); choose(e.dataTransfer.files?.[0]) }} className={`flex min-h-[290px] w-full flex-col items-center justify-center border-2 border-dashed px-6 text-center transition ${dragging ? 'border-primary bg-indigo-50' : 'border-border-strong bg-surface-soft hover:border-slate-400 hover:bg-white'}`} whileTap={{ scale: .995 }}>
